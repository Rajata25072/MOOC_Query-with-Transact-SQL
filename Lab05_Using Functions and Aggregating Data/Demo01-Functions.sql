...
-- Step Breakdown
-- Step1-3 Find position for '-' and substract from LEN
SELECT ProductNumber, RIGHT(ProductNumber, 3) AS Step1, REVERSE(RIGHT(ProductNumber, 3)) AS Step2,
CHARINDEX('-', REVERSE(RIGHT(ProductNumber, 3))) AS Step3,
SUBSTRING(ProductNumber, LEN(ProductNumber) - CHARINDEX('-', REVERSE(RIGHT(ProductNumber, 3))) + 2, 2) AS SizeCode
FROM SalesLT.Product;
...
-- cat.ParentProductCategoryID = 1 -> 'Bikes
-- = 2 -> Components 
-- and so on

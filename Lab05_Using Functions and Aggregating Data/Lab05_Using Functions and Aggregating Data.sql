...
-- Your reports are returning the correct records, but you would like to modify how these records 
-- are displayed. 
	...
	-- as upper case and a column named ApproxWeight with the weight of each product rounded to the nearest 
	-- whole unit.
	SELECT ProductID, 
	...
	-- and month in which Adventure Works started selling each product. The month should be displayed 
	-- as the month name (for example, ‘January’).
	...
	SELECT ProductID, 
		UPPER(Name) AS ProductName, 
		CAST(Weight AS INT) AS ApproxWeight ,
		YEAR(SellStartDate) AS SellStartYear, 
...
-- The sales manager would like a list of customers ranked by sales. 
	...
	-- highest TotalDue values from the SalesOrderHeader table.
	SELECT CompanyName, 
		TotalDue, 
...
-- The product manager would like aggregated information about product sales. 
	...
	-- as the sum of the LineTotal from the SalesLT.SalesOrderDetail table, with the results sorted 
	-- in descending order of total revenue.
	SELECT p.Name, 	SUM(LineTotal) AS RevenueTotal
	FROM SalesLT.Product p
	LEFT JOIN SalesLT.SalesOrderDetail od ON p.ProductID = od.ProductID
	GROUP BY p.Name
...
-- Next Steps
-- Well done! You’ve completed the lab, and you’re ready to learn how to nest queries 
-- within one another in Module 6 – Using Subqueries and APPLY in the Course Querying with Transact-SQL.